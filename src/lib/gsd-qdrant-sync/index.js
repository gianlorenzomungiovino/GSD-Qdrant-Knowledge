...
class GSDKnowledgeSync {
  ...
  toAbsolutePath(filePath) {
    ...
    return filePath.startsWith(this.gsdDir) || /^[A-Za-z]:[\\/]/.test(filePath)
      ? filePath
      : join(this.gsdDir, filePath);
  }
  ...
  buildSearchText({ type, ids, relativePath, body }) {
    const labels = [
      `type:${type}`,
  ...
  async processFile(filePath) {
    ...
    try {
      ...
      const type = this.inferType(absolutePath);
      const ids = this.extractIds(absolutePath);
      const relativePath = this.toRelativePath(absolutePath);
      const searchText = this.buildSearchText({ type, ids, relativePath, body });
      ...
      const doc = {
        ...
        metadata: {
          type,
          scope: this.inferScope(absolutePath, ids),
          path: relativePath,
  ...
  async syncAll() {
    ...
    const currentFiles = new Set(files.map((file) => this.toAbsolutePath(file)));
  ...
  startWatcher() {
    ...
    this.watcher = watch(this.gsdDir, { recursive: true }, (eventType, filePath) => {
