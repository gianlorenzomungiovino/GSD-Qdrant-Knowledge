...
class GSDKnowledgeSync {
  ...
  async init() {
    ...
    this.projectName = basename(this.projectRoot).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|$/g, '');
   ...
   * Creates unique collection names per project to avoid conflicts
   * Format: {project-name}-docs and {project-name}-snippets
   */
  getCollectionNames() {
    return {
  ...
  async ensureCollection() {
    ...
    // Create project-specific docs collection
    try {
      await this.client.getCollection(collections.docs);
    } catch (err) {
      if (err.status === 404) {
        await this.client.createCollection(collections.docs, {
          vectors: {
            [CONFIG.vectorName]: {
        ...
        console.log(`📦 Created project-scoped collection: ${collections.docs}`);
      } else {
        throw err;
      }
    }

    // Create project-specific snippets collection
    try {
      await this.client.getCollection(collections.snippets);
    } catch (err) {
      if (err.status === 404) {
        await this.client.createCollection(collections.snippets, {
          vectors: {
            [CONFIG.vectorName]: {
              size: CONFIG.vectorSize,
              distance: 'Cosine',
        ...
        console.log(`📦 Created project-scoped collection: ${collections.snippets}`);
      } else {
        throw err;
      }
    }
    
    // Store collection names in instance for later use
  ...
  async deleteDocument(filePath) {
    ...
    // Get the correct collection based on file extension
    const ext = extname(absolutePath).toLowerCase();
    const collection = ext === '.md' && filePath.includes(`${require('path').sep}.gsd${require('path').sep}`)
      ? (this.collections ? this.collections.docs : null)
      : (this.collections ? this.collections.snippets : null);
  ...
  async processFile(filePath, forcedType = null, extraMetadata = {}) {
    ...
    // Get project-scoped collection names if not already set
    if (!this.collections) {
      this.collections = this.getCollectionNames();
    }
  ...
  async upsertBatch(docs, collection) {
    ...
    for (let i = 0; i < points.length; i += CONFIG.batchSize) {
      ...
      console.log(`   📦 Batch ${batchNum}/${totalBatches}: Upserting ${chunk.length} points to ${collection}...`);
      await this.client.upsert(collection, { points: chunk });
  ...
  async syncAll() {
    ...
    console.log(`📦 Docs collection: ${collections.docs}`);
    console.log(`📦 Snippets collection: ${collections.snippets}`);
  ...
  async searchWithContext(query, options = {}) {
    const { collection = null, limit = 10, withContext = false } = options;
    
    // If collection not specified, use project-scoped snippets collection
    const targetCollection = collection || (this.collections ? this.collections.snippets : null);
    
    if (!targetCollection) {
      console.error('❌ No collection specified and project not initialized');
      return [];
    }
    ...
    let searchResults;
    try {
      searchResults = await this.client.search(targetCollection, {
        vector: {
          vector: queryVector,
...
async function runCLI() {
  ...
  await sync.init();
  await sync.ensureCollection(); // Ensure collections exist before searching
  ...
  if (command === 'sync') {
    ...
    const collection = collectionIndex > -1 && args[collectionIndex + 1] 
      ? args[collectionIndex + 1] 
      : (sync.collections ? sync.collections.snippets : null);
    
    const limitIndex = args.indexOf('--limit');
    const limit = limitIndex > -1 && args[limitIndex + 1] ? parseInt(args[limitIndex + 1], 10) : 10;
    
    const withContextIndex = args.indexOf('--with-context');
    const withContext = withContextIndex > -1;
    
    // Get query from remaining args
    const queryIndex = args.indexOf('QUERY');
    const query = queryIndex > -1 ? args[queryIndex + 1] : (args[1] || '');
    
    if (!query) {
      console.error('Usage: query QUERY [--collection COLLECTION] [--limit LIMIT] [--with-context]');
      process.exit(1);
    }
    
    console.log(`🔍 Searching for "${query}" in ${collection || 'project-scoped snippets'}...`);
    ...
    for (const result of results) {
      console.log(`\n  📄 ${result.path} (score: ${result.score.toFixed(3)}, type: ${result.type})`);
      if (result.context && result.context.length > 0) {
        console.log(`  🔗 Context from ${result.context.length} file(s)`);
      }
    }
  } else {
    console.error('Usage: sync|watch|query QUERY [--collection COLLECTION] [--limit LIMIT] [--with-context]');
    process.exit(1);
  }
}
